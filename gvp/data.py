...
class ProteinGraphDataset(data.Dataset):
    ...
    def _featurize_as_graph(self, protein):
        ...
        with torch.no_grad():
            ...
            sasa = torch.as_tensor(protein['SASA'],
                                    device=self.device, dtype=torch.float32)
            ...
            seq_arr = torch.zeros(seq.shape[0],20)
            for i in range(len(seq)):
                seq_arr[i,int(seq[i])] = 1;

            Y = torch.as_tensor([int(a) for a in protein['Y']],dtype=torch.long)

            mask = torch.isfinite(coords.sum(dim=(1,2)))
            coords[~mask] = np.inf

            #print("A",mask.shape,mask)
            ...
            #node_s = dihedrals
            #print(dihedrals.shape,seq_)
            #print("before,S:",dihedrals.shape,seq_arr.shape,sasa.unsqueeze(1).shape)

            node_s = torch.cat([dihedrals,seq_arr,sasa.unsqueeze(1)],dim=1)
            #node_s = dihedrals

            #print("before,V:",orientations.shape,sidechains.unsqueeze(-2).shape)
            node_v = torch.cat([orientations, sidechains.unsqueeze(-2)], dim=-2)


            edge_s = torch.cat([rbf, pos_embeddings], dim=-1)
            edge_v = _normalize(E_vectors).unsqueeze(-2)
            #print("NODE:",node_s.shape,node_v.shape)
            #print("EDGE:",edge_s.shape,edge_v.shape)

            node_s, node_v, edge_s, edge_v = map(torch.nan_to_num,
                    (node_s, node_v, edge_s, edge_v))

        data = torch_geometric.data.Data(x=X_ca, Y=Y, seq=seq, name=name,
    ...
    #Repeat the same input for all atoms
    def _repeat(self,X,n_atoms,three_dim=False):
        sz = X.shape;
        newX = torch.ones(sz[0]*n_atoms,sz[1]);
        if (three_dim):
            newX = torch.ones(sz[0]*n_atoms,sz[1],sz[2]);
        for i in range(sz[0]):
            for j in range(n_atoms):
                newX[i*n_atoms+j,...] = X[i,...]
        return newX;
