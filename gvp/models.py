...
class CPDModel(torch.nn.Module):
    ...
    def __init__(self, node_in_dim, node_h_dim,
        ...
        #self.W_s = nn.Embedding(20,1)
        edge_h_dim = (edge_h_dim[0] , edge_h_dim[1])
        #edge_h_dim = (edge_h_dim[0], edge_h_dim[1])
        ...
        #output 1 for 0-no glycan binds, 1-glycan binds
        #self.W_out = GVP(node_h_dim, (1, 0), activations=(None,None))
        self.Activate = nn.Sigmoid();
        self.W_out = GVP(node_h_dim, (1, 0),activations=(None,None))


    def forward(self, h_V, edge_index, h_E, label):
        '''
        ...
        '''
        #print(h_V[0].shape,h_V[1].shape)
        #print(h_E[0].shape,h_E[1].shape)
        h_V = self.W_v(h_V)
        h_E = self.W_e(h_E)
        ...
        for layer in self.decoder_layers:
            h_V = layer(h_V, edge_index, h_E, autoregressive_x = None)

        logits = self.W_out(h_V)
        # dlogits = self.Activate(logits)
        #print(logits.shape)
        #l = logits.detatch().cpu().numpy()
        #print(np.ptp(l))
        #print(logits)

        return logits

    def sample(self, h_V, edge_index, h_E, n_samples, temperature=0.000000001):
        '''
